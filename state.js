...
import { BlockElement, connectors, constructBlockData, GroupElement, unwrap} from "./block.js"
...
export let updateBuffers = () => {
	...
	edges.forEach(edge => {
		...
		if (found == -1){
			store.apply(EDGEMAP.concat(edge.fromNode), 'push', {edgeId: edge.id, blockId: edge.toNode})
			console.log("Connected ", edge.fromNode, ' to ', edge.toNode, store.get(EDGEMAP.concat(edge.fromNode))
)
		}
	})
